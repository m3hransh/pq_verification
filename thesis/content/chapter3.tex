...
In Chapter~\ref{ch:verification}, we will verify that these implementations satisfy the priority queue interface and maintain the leftist heap invariants.
...
The root of the resulting pennant is the minimum of the roots of the two original pennants.
...
Following the ripple-carry adder approach, we define a \texttt{bHalfAdder} function that takes two bits and produces a sum bit and a carry bit.
...
Additionally, we define a \texttt{bFullAdder} function that takes two bits and an incoming carry bit, producing a sum bit and an outgoing carry bit.
...
The \texttt{bSum} and \texttt{bCarry} functions handle the logic for combining bits of the same rank according to the rules of binary addition.
...
Rather than verifying every operation, we concentrate on the main operations, which serve as the fundamental building blocks from which other operations can be derived.
