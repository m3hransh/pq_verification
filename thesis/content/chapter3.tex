...
Instead of classical binomial trees, this implementation uses \emph{pennants}.

\begin{code}[caption={Pennant Data Type}, label={lst:pennant-type}]
  data BinTree a
    = Empty
    | Bin { value :: a, left :: BinTree a, right :: BinTree a, height :: Int }

  data Pennant a = P { root :: a, size :: Int, bin :: (BinTree a) }
\end{code}

A pennant's \texttt{size} corresponds to its height \(h\).
...
\begin{itemize}
    ...
    \item {\textbf{Perfect Binary Property}: The internal binary tree is a perfect binary tree of height \(k\), meaning all internal nodes have two children and all leaves are at the same level.}
\end{itemize}

\input{figures/binomial.tex}
The fundamental operation on pennants is \texttt{link}, which takes two pennants of the same rank \(k\) and merges them into a single pennant of rank \(k+1\).
The root of the new pennant is the smaller of the two original roots.

\begin{code}[caption={Linking two Pennants}, label={lst:pennant-link}]
  link :: (Ord a) => Pennant a -> Pennant a -> Pennant a
  link p1@(P r1 k t1) p2@(P r2 _ t2)
    | r1 <= r2  = P r1 (k + 1) (Bin r2 t2 t1 (k + 1))
    | otherwise = P r2 (k + 1) (Bin r1 t1 t2 (k + 1))
\end{code}

A binomial heap is then represented as a list of "bits", where each bit can be either \texttt{Zero} or \texttt{One}. A \texttt{One} bit at position \(k\) signifies the presence of a pennant of rank \(k\).
...
\begin{code}[caption={Half Adder for Binomial Bits}, label={lst:bit-half-adder}]
  halfAdder :: (Ord a) => BinomialBit a
                        -> BinomialBit a
                        -> (BinomialBit a, BinomialBit a)
  halfAdder b1 b2 = (sum b1 b2, carry b1 b2)
\end{code}
...
\begin{code}[caption={Full Adder for Binomial Bits}, label={lst:bit-adder}]
  fullAdder :: (Ord a) => BinomialBit a
                       -> BinomialBit a
                       -> BinomialBit a
                       -> (BinomialBit a, BinomialBit a)
  fullAdder b1 b2 cin = 
    let (s, c1) = halfAdder b1 b2
        (s', c2) = halfAdder s c
     in (s', sum c1 c2)
\end{code}

The sum and carry functions handle the logic for combining bits of the same rank according to the rules of binary addition.
\begin{code}[caption={Sum and Carry Functions}, label={lst:bit-sum-carry}]
  sum :: (Ord a) => BinomialBit a -> BinomialBit a -> BinomialBit a
  sum (Zero r) (Zero _) = Zero r
  sum (Zero r) (One _ p) = One r p
  sum (One _ p) (Zero r) = One r p
  sum (One r1 p1) (One r2 p2) = Zero r1
  carry :: (Ord a) => BinomialBit a -> BinomialBit a -> BinomialBit a
  carry (Zero r1) (Zero r2) = Zero (r1 + 1)
  carry (Zero r1) (One _ p2) = Zero (r1 + 1)
  carry (One _ p1) (Zero r2) = Zero (r2 + 1)
  carry (One r1 p1) (One _ p2) = One (r1 + 1) (link p1 p2)
...
\begin{code}[caption={Merging Two Binomial Heaps}, label={lst:heap-merge}]
  add :: (Ord a) => BinomialHeap a -> BinomialHeap a -> BinomialHeap a
...
\end{code}
    ...
    \item {\textbf{splitMin}: This is the most involved operation. First, the pennant with the minimum root is found and removed from the list of pennants.
Let's say it has rank \(k\). After removing the minimum element from the pennant, the perfect binary tree is then "dismantled". 
A perfect binary tree of height \(k-1\) can be split into list of pennants of ranks \(0\) to \(k-1\).
Removing the root leaves another binomial heaps (list of pennants), which are then merged back into the heap.
...
Rather than verifying every operation, we concentrate on the merge operation, which serves as the fundamental building block from which other operations can be derived.
