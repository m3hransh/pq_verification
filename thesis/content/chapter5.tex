...
\end{code}
\texttt{LiquidHaskell} can automatically verify that \texttt{x1} is a lower bound for \texttt{l1} since \texttt{l1} is a child of the original heap with root \texttt{x1}.
However, it cannot automatically verify that \texttt{x1} is a lower bound for the second argument, which is the result of \texttt{heapMerge r1 h2}.
...
This lemma states that if \texttt{x1} is a lower bound for \texttt{r1} and \texttt{x1 <= x2} where \texttt{x2} is a lower bound for \texttt{h2},
then \texttt{x1} must also be a lower bound for the result of merging \texttt{r1} and \texttt{h2}.

To prove this lemma, we perform case analysis on \texttt{r1}:
...
\item For the base case where \texttt{r1} is \texttt{EmptyHeap}, the result of \texttt{heapMerge EmptyHeap h2} is simply \texttt{h2}.
And since \texttt{x1 <= x2} and \texttt{x2} is a lower bound for \texttt{h2},
we can conclude using \texttt{lemma\_isLowerBound\_transitive} that \texttt{x1} is also a lower bound for \texttt{h2}.
...
\item For the inductive case where \texttt{r1} is a \texttt{HeapNode}, we again use
  \\ \texttt{lemma\_isLowerBound\_transitive}.
Additionally, we relay on mutual recursion between \texttt{heapMerge} and \texttt{lemma\_merge\_case1} to handle the recursive structure of the heaps.
...
\subsection*{Other Heap Operations}
