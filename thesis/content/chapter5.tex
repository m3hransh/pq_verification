...
that holds the remaining elements following Listing \ref{lst:pennant-type}. We expose its height to the logic and
...
perfect tree of the advertised height:
\begin{code}
data Pennant a 
  = P 
  { root    :: a
...
\begin{itemize}
    ...
    \item {\textbf{Perfect Binary Property}: 
            The structure of \texttt{BinTree} guarantees that it is a perfect binary tree.
            The refinement \texttt{right :: BinTreeHeight a (bheight left)} enforces that the left and right children of any node have the same height (\texttt{bheight left == bheight right}).
            The height of a node is then one greater than its children's height, which is the definition of a perfect binary tree.
            The \texttt{bheight b + 1 = pheight} check in the \texttt{Pennant} type connects the pennant's height to the height of its internal tree.}
...
\begin{code}
type BinomialHeap a 
  = PList <{\hd v -> rank hd == rank v - 1}> (BinomialBit a)
\end{code}
...
bCarry :: b1 : {BinomialBit a | rank b1 >= 0}
...
addWithCarry :: h1 : BinomialHeap a
  -> h2 : {BinomialHeap a | 
            ( bRank h2 == bRank h1 || isNil h1) 
            || isNil h2
          }
  -> carry : {BinomialBit a | ((not isNil h1) => rank carry == bRank h1)
