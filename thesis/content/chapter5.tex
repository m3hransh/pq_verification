...
This heap passes verification because the heap property holds ($1 \le 2$),
the leftist property holds ($\texttt{rrank right} = 0 \le 1 = \texttt{rrank left}$),
and the rank is correct ($1 = 1 + 0$).
...
LiquidHaskell rejects this because the heap property is violated: the parent value 5 is greater than the left child value 2, contradicting the refinement \texttt{left :: LeftistHeapBound a value} which requires \texttt{isLowerBound 5 (HeapNode 2 ...)}.
...
There is no invariant to maintain here, but we define a measure to help with other proofs.
\begin{code}
{-@ measure heapIsEmpty @-}
...
This can be directly extracted from the \texttt{HeapNode}, and \texttt{LiquidHaskell} can verify that this value is indeed a lower bound for the entire heap.
\begin{code}
...
Additionally, the termination metric must reflect which arguments are reduced in the recursive call.
...
inserting an element into a heap produces a new heap where
the inserted element is a lower bound for the resulting heap if it is smaller than the minimum of the original heap (when non-empty),
...
\end{code}
\texttt{LiquidHaskell} can automatically verify that the properties hold, given the correctness of \texttt{heapMerge}.
Therefore, no additional lemmas are required here.
...
provides a termination metric for recursive functions over heaps.
...
validBH = Cons (Zero 0)
                      (Cons (One 1 (P 3 1 (Bin 5 Empty Empty 0)))
                        (Cons (Zero 2) Nil))
\end{code}

This heap satisfies the invariant: the bits have ranks 0, 1, and 2, increasing by exactly one at each step.
The \texttt{One} bit at rank 1 contains a pennant of height 1, satisfying \texttt{pheight p == oorder}.
...
LiquidHaskell rejects this because the rank-incrementing invariant is violated: the first bit has rank 0,
but the second bit has rank 2, contradicting the refinement \texttt{rank (bhead bs) = rank hd + 1} which
requires the rank to increase by exactly one.

We define \texttt{heapIsEmpty} and \texttt{bhead} as measures to check emptiness and access the first bit:
