...
Haskell code \cite{LH2020}. Programmers can enrich type signatures with logical refinements,
...
invariants. During compilation, \texttt{LiquidHaskell} generates \emph{subtyping
	queries} from these annotations and delegates them to an SMT solver \cite{vazou_liquidhaskell_2014}. If the
...
\texttt{LiquidHaskell} (\S~\ref{sec:specification}), and discuss its strengths, limitations,
...
The Haskell type system ensures that \texttt{lookup} takes an integer, a list of integers, and
...
handling of invalid inputs to the caller, who must remember to check for \texttt{Nothing}.
...
type Nat =  {v : Int | v >= 0}
\end{code}
Additionally, holes can be used for Haskell types, as those types can be inferred from the
regular Haskell type signature or via GHC's type inference \cite{vazou_liquidhaskell_2014}.
...
\]
These logical formulas are then passed to an SMT solver for validation.
...
postconditions. For \texttt{lookup}, we can require that the index is non-negative and
...
The type of the second argument states that the list \texttt{xs} must have length greater than \texttt{i}.
\texttt{len} is a function defined by \texttt{LiquidHaskell} in the refinement logic that returns the length of the list.
In \S\ref{sec:reflection}, we will show how to define and use user-defined functions in the refinement logic.

\section{Refined Data Types}
\label{sec:lh-refined-datatypes}
In the previous examples, we saw how refinements of the input and output of functions allow us to make stronger arguments about our programs.
...
This refined \textit{Slist} data type ensures the stored \texttt{size} always matches the length of the \texttt{elems} list,
...
\section{Lifting Functions to the Refinement Logic}
...
\texttt{Measure} can be used on a function with one argument which is an Algebraic Data Type (ADT), such as a list \cite{niki_lecture_2024}.
...
This has the advantage of allowing the user to lift into the logic functions with more than one argument, but the verification
is no longer automated \cite{niki_lecture_2024}.
...
(x : xs) ++ ys = x : (xs ++ ys)
\end{code}
The \texttt{\{-@ LIQUID "--reflection" @-\}} annotation is used to activate the reflection feature in \texttt{LiquidHaskell}.
The \texttt{reflect} annotation lifts the \texttt{(++)} into the logic in three steps \cite{vazou2018}:
\begin{enumerate}
...
\end{code}
	...
	\item \textbf{Application}: With the reflected refinement type, each application of \texttt{(++)}
	      ...
	      next section, we will look into \texttt{PLE} that allows unfolding the definition of the function multiple times.
\end{enumerate}
We can now reason about properties of \texttt{(++)} in the refinement logic that require unfolding its definition,
...
\section{Refinement Abstraction}
...
\texttt{LiquidHaskell} can verify that the following list is sorted:
...
Similarly, \texttt{[n, 1]} is greater than \texttt{[n, 0]} in the call from \texttt{isOdd} to \texttt{isEven}.
...
\section{Proof by Logical Evaluation}
\label{sec:lh-ple}
...
We can activate PLE by adding the \texttt{\{-@ LIQUID "--ple" @-\}} annotation to automate most parts of the proof for associativity of \texttt{(++)}:
